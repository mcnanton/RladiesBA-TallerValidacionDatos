...
Devuelve todo registro duplicado

```{r}
duplicados <- dataset_sucio[duplicated(dataset_sucio) | duplicated(dataset_sucio, fromLast = TRUE), ]
print(duplicados)

```

```{r}
anyDuplicated(dataset_sucio, fromLast = T)
```
...
Podemos usar la funcion rows_distinct que se puede aplicar directamente sobre el agente (como mostramos en este ejemplo) pero que tambien cuenta con versiones de testeo como test_row_distinct y expect_rows_distinct
...
Estas mismas transformaciones que creamos arriba con `across` podriamos aplicarlas como reglas usando el paquete validate en el que

\- Indique los valores NA por la media de cada columna

\- Indique cuantos casos superan por un factor de 1.5 veces el rango intercuartílico
...
#    mutate(
#     # arreglamos especie
#     especie = str_replace_all(especie, "[^a-zA-ZáéíóúÁÉÍÓÚ]", ""),
#     especie = str_replace_all(especie, "(?i)adelia", "Adelia"),
#     especie = str_replace_all(especie, "(?i)papúa|papua", "Papúa"),
#     especie = str_replace_all(especie, "(?i)barbijo", "Barbijo"),
#     especie = as.factor(especie),
#     
#     # arreglamos isla
#     isla = str_replace_all(isla, "[^a-zA-Z]", ""),
#     isla = str_replace_all(isla, "(?i)dream", "Dream"),
#     isla = str_replace_all(isla, "(?i)torgersen", "Torgersen"),
#     isla = str_replace_all(isla, "(?i)biscoe", "Biscoe"),
#     isla = as.factor(isla),
#     
#     # arreglamos sexo
#     sexo = str_replace_all(sexo, "[^a-zA-Z]", ""),
#     sexo = str_replace_all(sexo, "(?i)macho", "macho"),
#     sexo = str_replace_all(sexo, "(?i)hembra", "hembra"),
#     sexo = if_else(sexo == "NA", NA_character_,sexo),
#     sexo = as.factor(sexo),
#     
#     # arreglamos anio 
#     anio = str_replace_all(anio, "[^0-9]", ""),  
#     anio = case_when(
#       anio %in% c("2007","20070") ~  "2007",
#       anio %in% c("20080") ~ "2008",
#       anio %in% c("20090") ~"2009",
#       TRUE ~anio),
#     anio = ifelse(anio %in% c("2007", "2008", "2009"), anio, NA),  
#     anio = as.factor(anio)  
#   ) |> 
...
Con validate podemos chequear de una lista de códigos con
...
Betsy: Como hacer para que no detenga el analisis en el primer assert(in_set) ver si aplicar in_set()
